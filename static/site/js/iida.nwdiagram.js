    ...
    iida.nwdiagram = function () {
        ...
        ["idPhysical", "idTopology"].forEach(id => {
            ...
            a.addEventListener("click", function (evt) {
                evt.stopPropagation();
                evt.preventDefault();
                document.getElementsByName("dataChange").forEach(element => { element.classList.remove("active"); });
        ...
        function getInitialPosition(node) { return node.data("initialPosition") || { x: 0, y: 0 }; }
            ...
            routers.forEach(router => {
                ...
                ports.forEach(p => {
                    ...
                    if (port) {
                        ...
                        port.connectedEdges().forEach(edge => { showHideElement(edge, show); });
                    }
                });
                router.connectedEdges().forEach(edge => { showHideElement(edge, show); })
        ...
        var CyShortestPath = (function () {
            var _dijkstra = function (cy, startNodeId) {
                ...
                CyShortestPath.dijkstraResult = cy.elements().not(".disabled").dijkstra(startNode, function (edge) {
